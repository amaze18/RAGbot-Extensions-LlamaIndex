...
if st.session_state.messages[-1]["role"] != "assistant":
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            response_time = evaluate_response_time(chat_gpt, prompt)

            # Get the reference response
            reference_response = get_reference_response(prompt)

            # Evaluate the response and get ROUGE scores
            rouge_scores = calculate_rouge_scores(answer, reference_response)
            ...
            st.write("ROUGE-1 (Recall):", rouge_scores[0]["rouge-1"]["r"])
            st.write("ROUGE-L (Recall):", rouge_scores[0]["rouge-l"]["r"])

            st.write("PRINTING CONTEXT")
            for i in range(3):
